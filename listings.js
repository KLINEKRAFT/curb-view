...
export default async function handler(req) {
  ...
  let feedUrl = (typeof process !== 'undefined' && process.env && process.env.FEED_URL) || FEED_URL;
  feedUrl = (feedUrl || '').trim();

  // Diagnostic info — what does the edge function actually see?
  const diag = {
    hasFeedUrl: !!feedUrl,
    feedUrlLength: feedUrl.length,
    feedUrlScheme: feedUrl.startsWith('https://') ? 'https'
                  : feedUrl.startsWith('http://') ? 'http'
                  : feedUrl ? 'other' : 'empty',
    hasProcessEnv: typeof process !== 'undefined' && !!process.env,
    runtime: 'edge',
  };

  // No feed URL configured → return mock so UI works in dev
  if (!feedUrl) {
    return json({ source: 'mock', listings: MOCK_LISTINGS, count: MOCK_LISTINGS.length, diag });
  }

  const now = Date.now();
  if (!debug && cache.data && (now - cache.timestamp) / 1000 < CACHE_TTL_SECONDS) {
    return json({ source: 'cache', ...cache.data });
  }

  // Try the configured URL first. If it's http://, also try the https:// variant
  // since some edge runtimes block plaintext HTTP.
  const urlsToTry = [feedUrl];
  if (feedUrl.startsWith('http://')) {
    urlsToTry.push('https://' + feedUrl.slice(7));
  }
  ...
  for (const tryUrl of urlsToTry) {
    try {
      const response = await fetch(tryUrl, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; CBSelect-CurbView/1.0)',
          'Accept': 'application/xml,text/xml,*/*',
        },
        redirect: 'follow',
      });
      attempts.push({ url: tryUrl, status: response.status, ok: response.ok });
      if (!response.ok) continue;
      const xml = await response.text();
      if (!xml || xml.length < 100) {
        attempts[attempts.length - 1].error = `Empty body (${xml.length} bytes)`;
        continue;
      }
      ...
      return json({
        ...
        ...(debug ? { diag, attempts, urlUsed: tryUrl, xmlBytes: xml.length } : {}),
      });
    } catch (err) {
      attempts.push({ url: tryUrl, error: String(err.message || err) });
    }
  }
